...
3. 切り抜き比率、リサイズする辺とピクセル数、JPEG 画質、メタデータ、ファイル名サフィックスを調整します。
4. ［圧縮を開始］を選びます。
5. 完了した画像の行から出力ファイルを開くか、保存先フォルダを表示します。

切り抜き位置は常に中央です。  
元画像の縦横比を維持するほか、プリセットまたは任意の横対縦の比率を指定できます。  
リサイズは選択した幅または高さを基準とし、クロップ後の縦横比を維持します。

ウィンドウが小さい場合は、設定と変換処理プレビューをスクロールして確認できます。  
画像追加と圧縮開始のボタンは、スクロール中も表示されます。

比率とリサイズする辺は、選択欄をクリックして候補一覧から選びます。  
...
1. ImageSquoosher 右上の［画像を追加］の隣にある Finder 連携ボタンを選びます。
