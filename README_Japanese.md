...
3. アスペクト比、リサイズする辺とピクセル数、JPEG 画質、メタデータ、ファイル名サフィックスを調整します。
4. ［変換開始］を選びます。
...
ウィンドウが小さい場合は、設定と［変換する画像］の一覧をスクロールして確認できます。  
［画像を追加］は一覧の［すべて外す］の隣にあり、［変換開始］と［停止］は画面下部に表示されます。

各画像は、上段に入出力ファイル名とサイズ、下段に変換前後の寸法とアスペクト比を表示します。  
変換が完了すると圧縮率と進捗バーが黄緑色で残り、一覧の高さを保ったまま結果を比較できます。  
保存先フォルダは変換前から開けます。
