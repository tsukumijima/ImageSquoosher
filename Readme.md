...
上書きがオンの間は、画質の下にあるサフィックス欄が無効になります。  
...
右上のパズルボタン［Explorer の右クリックメニューに追加］を押し、Windows の確認画面で登録を承認します。  
Explorer で JPEG・PNG・WebP を選択し、右クリックメニューの［ImageSquoosher］を選ぶと、選択した画像を一覧に追加します。  
Windows 11 では、最初に開く右クリックメニューへ直接表示されます。  
Windows 10 の項目名は［ImageSquoosher で圧縮・リサイズ］です。

変換中に選択画像を送った場合は、処理の完了または停止後に届いた画像をまとめて追加します。  
［変換開始］を押すと、追加した画像を変換できます。

Windows 11 の登録は現在のユーザーに対して行い、必要な場合だけ管理者の承認を求めます。  
登録中は開発者モードを一時的に有効にし、処理後に元の設定へ戻します。  
通常の画像変換や右クリックからの起動には管理者権限が不要です。  
Windows 10 では現在のユーザーの従来型メニューへ登録します。

登録済みのボタンは［Explorer の右クリックメニューから削除］に切り替わります。  
アプリのフォルダを移動した場合は、移動先で［Explorer の右クリック連携を修復］を押してください。  
アプリを削除する前にこのボタンで連携を解除すると、右クリック項目を削除できます。

配布物は ZIP のままで、展開したフォルダ内の `AppxManifest.xml` を Windows に登録します。  
この登録方法は Windows の開発用途の機能を利用するため、組織のポリシーで開発者モードが禁止されている環境では登録に失敗することがあります。
