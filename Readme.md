...
右上のパズルボタンから［Explorer 連携を管理］を開き、［右クリックメニューに追加する］をオンにします。  
Explorer で JPEG・PNG・WebP を選択し、右クリックメニューの［ImageSquoosher で圧縮・リサイズ］を選ぶと、選択した画像を一覧に追加します。  
Windows 11 では［その他のオプションを確認］からこの項目を選べます。

変換中に選択画像を送った場合は、処理の完了または停止後に届いた画像をまとめて追加します。  
［変換開始］を押すと、追加した画像を変換できます。

連携は現在のユーザーに登録され、管理者権限は不要です。  
アプリのフォルダを移動した場合は、移動先のアプリで連携を有効にしてください。  
アプリを削除する前に、この設定をオフにすると右クリック項目を解除できます。
